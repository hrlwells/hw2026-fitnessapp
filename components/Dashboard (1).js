...
function SignIn() {
  ...
  const signInPassword = async () => {
    ...
    // On success the auth listener swaps in the app.
  };
  ...
  const inputStyle = { width: "100%", padding: "12px 14px", borderRadius: 11, border: `1px solid ${C.line}`, background: C.paper, fontFamily: sans, fontSize: 15, color: C.ink, outline: "none", boxSizing: "border-box" };
  const btnStyle = (disabled) => ({ width: "100%", marginTop: 10, padding: "12px 14px", borderRadius: 11, border: "none", background: disabled ? C.line : C.ink, color: disabled ? C.faint : C.paper, fontFamily: sans, fontWeight: 600, fontSize: 14.5, cursor: disabled ? "default" : "pointer" });
  const linkBtn = { marginTop: 14, background: "none", border: "none", color: C.slate, fontFamily: sans, fontSize: 12.5, cursor: "pointer", textDecoration: "underline", padding: 0 };
        ...
        {mode === "link" && sent ? (
          <div style={{ marginTop: 20 }}>
            <div style={{ background: C.greenSoft, color: C.green, borderRadius: 12, padding: "12px 14px", fontSize: 13.5, lineHeight: 1.5 }}>
              <b>Check your email.</b> Tap the link to sign in. Note it opens in your browser — if you're in the home-screen app, use a password instead.
            </div>
            <button onClick={() => { setSent(false); setErr(null); }} style={linkBtn}>Back</button>
          </div>
        ) : (
          <div style={{ marginTop: 20 }}>
            ...
            <button
              ...
              style={btnStyle(busy || !email.trim() || (mode === "password" && !password))}
            >
            ...
            <button
              onClick={() => { setMode(mode === "password" ? "link" : "password"); setErr(null); }}
              style={linkBtn}
            >
...
function PasswordSetter() {
  ...
  const save = async () => {
    ...
    setBusy(false);
    if (error) setErr(error.message);
    else { setMsg("Password saved. Use it to sign in on your phone."); setPw(""); }
  };

  return (
    <div style={{ marginTop: 22, background: C.card, border: `1px solid ${C.line}`, borderRadius: 12, padding: "14px 16px" }}>
      <div style={{ fontFamily: serif, fontSize: 16, fontWeight: 600 }}>Password</div>
      <div style={{ fontSize: 12.5, color: C.slate, marginTop: 4, lineHeight: 1.5 }}>
        Set a password so you can sign in on your phone's home-screen app without an email link.
      </div>
      <input
        type="password"
        value={pw}
        ...
        style={{ width: "100%", marginTop: 10, padding: "11px 13px", borderRadius: 10, border: `1px solid ${C.line}`, background: C.paper, fontFamily: sans, fontSize: 14.5, color: C.ink, outline: "none", boxSizing: "border-box" }}
      />
      <button
        onClick={save}
        disabled={busy || !pw}
        style={{ width: "100%", marginTop: 8, padding: "11px", borderRadius: 10, border: "none", background: busy || !pw ? C.line : C.ink, color: busy || !pw ? C.faint : C.paper, fontFamily: sans, fontWeight: 600, fontSize: 13.5, cursor: busy || !pw ? "default" : "pointer" }}
      >{busy ? "Saving…" : "Save password"}</button>
      {msg && <div style={{ marginTop: 10, background: C.greenSoft, color: C.green, borderRadius: 9, padding: "8px 11px", fontSize: 12.5 }}>{msg}</div>}
      {err && <div style={{ marginTop: 10, background: C.berrySoft, color: C.berry, borderRadius: 9, padding: "8px 11px", fontSize: 12.5 }}>{err}</div>}
    </div>
  );
}

/* ---------------------------------------------------------------- shared */const navBtn = { background: C.card, border: `1px solid ${C.line}`, borderRadius: 10, width: 38, height: 38, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: C.ink };
